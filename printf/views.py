...
def addmerchant(request,new_user):
    ...
    return render(request, 'printf/addmerchant.html', {'merchantform': merchantform})
...
def updatemerchant(request):
    id = User.objects.get(id=request.user.id)
    merchant = Merchants.objects.get(user_id=id)
    return HttpResponseRedirect('/printf/updateMerchant/'+str(merchant.id))
